...
async function sendMessage(content?: string) {
  ...
  try {
    // 调用 AI 服务（非流式）
    const response = await window.aiBridge?.chat({
    ...
    if (response?.success) {
      assistantMessage.content = response.content || '抱歉，我无法生成回复。';
    } else {
      assistantMessage.content = `错误: ${response?.error || '请求失败'}`;
    }
  } catch (error) {
...
<template>
  ...
  >
    <div class="h-full w-96 bg-base-200 shadow-xl flex flex-col border-l border-base-300">
      <!-- 头部 -->
