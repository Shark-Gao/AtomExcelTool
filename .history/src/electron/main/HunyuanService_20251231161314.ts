...
// 系统脚本原子元数据结构
interface SystemScriptAtomMeta {
  FunctionName: string;
...
export class HunyuanService {
  ...
  constructor(config: HunyuanConfig) {
    ...
    // 自动加载原子知识库
    this.loadAtomKnowledgeFromConfig();
  }

  /**
   * 从配置文件加载原子知识库
   */
  private loadAtomKnowledgeFromConfig(): void {
    try {
      // 获取配置文件路径
      ...
      const decoratorPath = path.join(basePath, 'AtomDecoratorMetaData.json');
      const systemScriptPath = path.join(basePath, 'AtomSystemScriptMetaData.json');

      console.log('[HunyuanService] Loading atom knowledge from:', basePath);

      let decoratorAtoms: Record<string, DecoratorAtomMeta> = {};
      let systemScriptAtoms: Record<string, SystemScriptAtomMeta> = {};

      // 加载装饰器原子
      if (fs.existsSync(decoratorPath)) {
        const content = fs.readFileSync(decoratorPath, 'utf-8');
        decoratorAtoms = JSON.parse(content);
        console.log('[HunyuanService] Loaded decorator atoms:', Object.keys(decoratorAtoms).length);
      }

      // 加载系统脚本原子
      if (fs.existsSync(systemScriptPath)) {
        const content = fs.readFileSync(systemScriptPath, 'utf-8');
        systemScriptAtoms = JSON.parse(content);
        console.log('[HunyuanService] Loaded system script atoms:', Object.keys(systemScriptAtoms).length);
      }

      // 构建知识库
      this.buildKnowledgeBase(decoratorAtoms, systemScriptAtoms);
      this.atomKnowledgeLoaded = true;

    } catch (error) {
      console.error('[HunyuanService] Failed to load atom knowledge:', error);
      // 使用默认的系统提示词
  ...
  private buildKnowledgeBase(
    decoratorAtoms: Record<string, DecoratorAtomMeta>,
    systemScriptAtoms: Record<string, SystemScriptAtomMeta>
  ): void {
    // 按类别分组装饰器原子
    const categoryMap = new Map<string, DecoratorAtomMeta[]>();
    
    for (const [, meta] of Object.entries(decoratorAtoms)) {
    ...
    // 构建装饰器原子摘要
    let decoratorSummary = '';
    for (const [category, atoms] of categoryMap) {
      decoratorSummary += `\n### ${category}\n`;
      
      for (const atom of atoms) {
        decoratorSummary += `- **${atom.displayName || atom.className}** (\`${atom.className}\`)\n`;
        if (atom.description) {
          decoratorSummary += `  - 说明: ${atom.description}\n`;
        }
        if (atom.richDescription) {
          decoratorSummary += `  - 用法: ${atom.richDescription}\n`;
        }
        if (atom.baseClass) {
          decoratorSummary += `  - 基类: ${atom.baseClass}\n`;
        }
      }
    }

    // 构建系统脚本原子摘要（按函数名分组）
    let systemScriptSummary = '\n### 系统脚本原子\n';
    const funcGroups = new Map<string, SystemScriptAtomMeta[]>();
    
    for (const [, meta] of Object.entries(systemScriptAtoms)) {
      if (meta.FunctionName.startsWith('__')) continue; // 跳过测试原子
      
      const funcName = meta.FunctionName;
      if (!funcGroups.has(funcName)) {
        funcGroups.set(funcName, []);
      }
      funcGroups.get(funcName)!.push(meta);
    }

    for (const [funcName, atoms] of funcGroups) {
      const firstAtom = atoms[0];
      systemScriptSummary += `- **${funcName}** (\`${firstAtom.AtomClassName}\`)\n`;
      
      if (firstAtom.ParameterList && firstAtom.ParameterList.length > 0) {
        const params = firstAtom.ParameterList.map(p => {
          const optional = p.bOptional ? '?' : '';
          return `${p.ParameterName}${optional}: ${p.TypeNodeText || p.TypeString || 'any'}`;
        }).join(', ');
        systemScriptSummary += `  - 参数: (${params})\n`;
      }
    }
...
3. 如果用户的需求不明确，主动询问细节
4. 对于复杂配置，分步骤说明
5. 引用原子时使用反引号包裹类名，如 \`BoolTriggerValueIsInFubenDelegate\`
...
请根据用户的问题进行回答。`;
  ...
  /**
   * 初始化系统提示词，注入原子知识库（兼容旧接口）
   */
  initializeWithAtomKnowledge(metadata: ClassMetadata[]): void {
    // 如果已经从配置文件加载了知识库，跳过
    if (this.atomKnowledgeLoaded) {
      console.log('[HunyuanService] Atom knowledge already loaded from config files');
      return;
    }
