...
<template>
  <div v-if="isOpen" class="modal modal-open">
    <div class="modal-box w-full max-w-2xl max-h-[80vh] flex flex-col">
      ...
      <!-- P4V 设置标签页 -->
      <div v-if="activeTab === 'p4'" class="space-y-4">
        <div class="card bg-base-200">
          <div class="card-body p-4 space-y-4">
            <h3 class="card-title text-sm">Perforce 连接配置</h3>
            
            <!-- P4PORT -->
            <div class="form-control">
              <label class="label">
                <span class="label-text font-semibold text-sm">Server (Port)</span>
              </label>
              <input
                type="text"
                class="input input-bordered input-sm w-full"
                v-model="p4Port"
                placeholder="例如: ssl:9.134.225.161:1666"
                @blur="saveP4Settings"
              />
              <p class="text-xs text-base-content/60 mt-1">P4 服务器地址，格式: [ssl:]host:port</p>
            </div>

            <!-- P4USER -->
            <div class="form-control">
              <label class="label">
                <span class="label-text font-semibold text-sm">User</span>
              </label>
              <input
                type="text"
                class="input input-bordered input-sm w-full"
                v-model="p4User"
                placeholder="例如: sharkgao"
                @blur="saveP4Settings"
              />
              <p class="text-xs text-base-content/60 mt-1">P4 用户名</p>
            </div>

            <!-- P4CLIENT -->
            <div class="form-control">
              <label class="label">
                <span class="label-text font-semibold text-sm">Workspace (Client)</span>
              </label>
              <input
                type="text"
                class="input input-bordered input-sm w-full"
                v-model="p4Client"
                placeholder="例如: MHA_Client_main"
                @blur="saveP4Settings"
              />
              <p class="text-xs text-base-content/60 mt-1">P4 工作区名称</p>
            </div>

            <!-- 测试连接按钮 -->
            <div class="form-control">
              <button 
                class="btn btn-sm btn-outline btn-primary w-32"
                :disabled="p4Testing || !p4Port || !p4User || !p4Client"
                @click="testP4Connection"
              >
                <span v-if="p4Testing" class="loading loading-spinner loading-xs"></span>
                <span v-else>测试连接</span>
              </button>
              <div v-if="p4TestResult" class="mt-2">
                <div 
                  class="alert alert-sm py-2"
                  :class="p4TestResult.success ? 'alert-success' : 'alert-error'"
                >
                  <span class="text-xs">{{ p4TestResult.message }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card bg-base-200">
          <div class="card-body p-4 space-y-4">
            <h3 class="card-title text-sm">文件操作设置</h3>
            
            <div class="form-control">
              <label class="label cursor-pointer">
                <span class="label-text font-semibold text-sm">打开文件时提示 Checkout</span>
                <input
                  type="checkbox"
                  class="toggle toggle-primary toggle-sm"
                  :checked="p4CheckoutPromptEnabled"
                  @change="(event) => handleP4CheckoutPromptToggle((event.target as HTMLInputElement).checked)"
                />
              </label>
              <p class="text-xs text-base-content/60 mt-1">启用后，打开 P4 工程下的文件时会询问是否 Checkout</p>
            </div>
          </div>
        </div>
      </div>
